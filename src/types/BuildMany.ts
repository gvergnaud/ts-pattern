import type { Compute, Iterator, UpdateAt } from "./helpers.ts";
...
type Update<data, value, path> = path extends readonly [
  ...
  ? data extends readonly [any, ...any]
    ...
    : never
  : data extends readonly (infer a)[] ? Update<a, value, tail>[]
  : data extends Set<infer a> ? Set<Update<a, value, tail>>
  : data extends Map<infer k, infer v> ? Map<k, Update<v, value, tail>>
  : head extends keyof data ? Compute<
