...
import type { AnyMatcher, Matcher, Pattern } from "./Pattern.ts";
import type {
  Equal,
  IsUnion,
  MergeUnion,
...
> = i extends readonly (infer iItem)[] ? p extends readonly [] ? output
  : p extends readonly [infer p1, ...infer pRest]
    ? i extends readonly [infer i1, ...infer iRest] ? FindSelectionUnionInArray<
        iRest,
        pRest,
        [...path, p["length"]],
  ...
  : p extends readonly [...infer pInit, infer p1]
    ? i extends readonly [...infer iInit, infer i1] ? FindSelectionUnionInArray<
        iInit,
        pInit,
        [...path, p["length"]],
        output | FindSelectionUnion<i1, p1, [...path, p["length"]]>
