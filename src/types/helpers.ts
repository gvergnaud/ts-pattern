...
) extends (k: infer intersection) => void ? intersection
  : never;
...
export type Drop<
  xs extends readonly any[],
  n extends any[],
> = Length<n> extends 0 ? xs
  : xs extends readonly [any, ...infer tail] ? Drop<tail, Prev<n>>
  : [];
...
export type IsPlainObject<o, excludeUnion = BuiltInObjects> = o extends object
  ...
  ? o extends string | excludeUnion ? false
  : true
  : false;
...
export type IsLiteral<a> = [a] extends [null | undefined] ? true
  : [a] extends [string] ? string extends a ? false
    : true
  : [a] extends [number] ? number extends a ? false
    : true
  : [a] extends [boolean] ? boolean extends a ? false
    : true
  : [a] extends [symbol] ? symbol extends a ? false
    : true
  : [a] extends [bigint] ? bigint extends a ? false
    : true
  : false;
...
export type MergeUnion<a> =
  | {
    [k in AllKeys<a>]: a extends any ? k extends keyof a ? a[k]
      : never
      : never;
  }
  | never;
...
export type IsReadonlyArray<a> = a extends readonly any[] ? a extends any[] // mutable array
    ? false
  : true
  : false;
...
export type GetKey<O, K> = O extends any ? K extends keyof O ? O[K]
  : never
  : never;
