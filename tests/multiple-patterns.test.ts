...
Deno.test("exhaustive patterns should match if one of the patterns matches", () => {
  const testFn = (input: Option<number>) =>
    match(input)
      .with(
        { kind: "some", value: 2 as const },
...
Deno.test("no patterns shouldn't typecheck", () => {
...
Deno.test("should work with literal types", () => {
...
Deno.test("should work with nullables", () => {
...
Deno.test("should work with all types of input", () => {
  type Input =
    | null
    | undefined
    | number
...
Deno.test("when 2 returned values don't match, the error should be at the second returned value", () => {
...
Deno.test("issue #74: inference must work on every pattern in the list", () => {
