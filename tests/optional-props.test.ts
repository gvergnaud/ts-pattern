...
Deno.test("matching on optional properties should work", () => {
  type Post = {
    type: "post";
    id?: number;
    body: string;
...
Deno.test("should correctly narrow the input type when the input is assignable to the pattern type", () => {
  type Foo =
    | { type: "test"; id?: string }
...
Deno.test("issue #142: When pattern matching on an optional property, other optional properties should remain on the object", () => {
  enum SomeEnum {
    Foo = "Foo",
    Bar = "Bar",
  }
