...
import type { BigUnion, Event, Option, State } from "./types-catalog/utils.ts";
...
Deno.test("should work with non-unions", () => {
  match<number>(2)
    .with(2, () => "two")
    .with(3, () => "three")
    // @ts-expect-error
...
Deno.test("should work with object properties union", () => {
...
Deno.test("should work with lists", () => {
  type Input =
    | {
      type: "a";
...
Deno.test("should support P.any in a readonly tuple", () => {
...
Deno.test("should work with Sets", () => {
...
Deno.test("should work with Maps", () => {
...
Deno.test("should work with structures with a lot of unions", () => {
...
Deno.test("should work with generics in type guards", () => {
  const map = <A, B>(
    option: Option<A>,
    mapper: (value: A) => B,
  ): Option<B> =>
...
Deno.test("should work with inputs of varying shapes", () => {
...
Deno.test("should infer literals as literal types", () => {
...
Deno.test("should correctly exclude cases if when pattern contains a type guard", () => {
  match<{ x: 1 | 2 | 3 }>({ x: 2 })
...
Deno.test("should correctly exclude cases if .when is a type guard", () => {
...
Deno.test("should correctly exclude cases if the pattern is a literal type", () => {
...
Deno.test("should not exclude cases if the pattern is a literal type and the value is not", () => {
  match<{ x: number }>({ x: 2 })
    .with({ x: 2 }, ({ x }) => {
      type t = Expect<Equal<typeof x, 2>>;
      return "";
...
Deno.test("select should always match", () => {
...
Deno.test("should work with a single not pattern", () => {
...
Deno.test("should work with several not patterns", () => {
  const reducer = (state: State, event: Event): State =>
    match<[State, Event], State>([state, event])
      .with(
  ...
  const allPossibleCases = flatMap(
    range,
    (x) => flatMap(range, (y) => flatMap(range, (z) => [[x, y, z]] as const)),
  );
...
Deno.test("should work with not patterns and lists", () => {
  const f = (input: (1 | 2 | 3)[]) =>
    match(input)
      .with([P.not(1)], (x) => "ok")
      .with([1], (x) => "ok")
...
Deno.test("shouldn't exclude cases if the pattern contains unknown keys", () => {
  type Person = {
    sex: "a" | "b";
    age: "c" | "d";
  };
