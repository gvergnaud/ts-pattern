...
Deno.test("should work in a nested structure", () => {
...
Deno.test("should discriminate union types correctly", () => {
  const one = "one";
  const two = "two";
...
Deno.test("should discriminate union types contained in objects correctly", () => {
  const one = "one";
  const two = "two";
...
Deno.test("should correctly invert the type of a Matcher", () => {
...
Deno.test("should correctly exclude unit types with the unit wildcard", () => {
  expect(
...
Deno.test("shouldn't change a the type if its any or unknown", () => {
  expect(
...
Deno.test("should successfully exclude cases ", () => {
  const f = (
...
Deno.test("should consider the expression exhaustive if the sub pattern matches something that will never match", () => {
  expect(
    match<{ str: string }>({ str: "hello" })
...
Deno.test("issue #138 — P.not on literals shouln't exclude the whole primitive type.", () => {
  type Input =
...
Deno.test("shouldn't consider unexhaustive patterns exhaustive", () => {
...
Deno.test("exhaustive should work when P.not is followed by the anti-pattern", () => {
  match<number>(1)
