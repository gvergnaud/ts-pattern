...
import type { NonNullablePattern } from "../src/types/Pattern.ts";
...
Deno.test("should correctly extract the matching value from the input and an inverted pattern", () => {
...
Deno.test("should use the type of the pattern if the input is any or never", () => {
  type cases = [
    Expect<
      Equal<
...
Deno.test("should return the input type when pattern is unknown", () => {
  type cases = [
    Expect<
      Equal<
...
Deno.test("should return the correct branch a union based on the pattern", () => {
  type cases = [
    Expect<
      Equal<
        ExtractPreciseValue<
...
Deno.test("should support readonly input types", () => {
  type cases = [
    Expect<
      Equal<
        ExtractPreciseValue<
...
Deno.test("should work if the input type contains anys", () => {
...
Deno.test("should work with arrays", () => {
  type res1 = ExtractPreciseValue<
...
Deno.test("should pick the input type as the upper bound, even if it is assignable to the pattern type", () => {
...
Deno.test("should keep optional properties if they are optional on both `a` and `b`", () => {
  type Input =
    | {
      type: "a";
...
Deno.test("should exclude objects if the absent", () => {
...
Deno.test("should keep empty objects if they come from the input type", () => {
  type res1 = ExtractPreciseValue<
    { a: string } | { b: {} },
    { b: nonNullable }
  >;
...
Deno.test("should exclude objects even if the non-nullable key is deeply nested", () => {
...
Deno.test("Type narrowing should correctly work on branded strings", () => {
...
Deno.test("Type narrowing should correctly work on class instances", () => {
  class A {
    a = "a";
  }
  class B {
...
Deno.test("issue #63: it should correctly narrow Error subclasses", () => {
...
Deno.test("shouldn't get stuck on generics in the input structure that aren't matched by the pattern", () => {
